...
var visualscript  = module("ll\\visualscript");
...
function ReadCEntity(e)
{
	...
	e.ReadU8("physics_flags", "bool8"); // is_physics, after_detach_physics_on, sleeping, kinematic, force_kinematic, block_breacking, raycast, block_ai_los
...
function ReadUObject_Restrictor(e)
{
	...
	// old version format
	//e.ReadString("shape_type");
	//e.ReadFP32("shape_sphere_radius");
	//e.ReadVec3("shape_box_size");
...
function ReadInventoryItemObject(e, float_names, int_names)
{
	...
	// имена могут быть другими в зависимости от класса, потом допилю
	for(var i = 0; i < 6; i++)
	{
		var n;
...
function ReadHuman(e, brain_unit_read_func)
{
	...
	if(entity_ver >= ENTITY_VER_29)
	{
		...
		e.ReadBool("update_corpse_luminocity");
